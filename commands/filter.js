...
module.exports = {
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!queue || !queue?.playing) return interaction?.reply({ content: '⚠️ Музыка не играет!!', ephemeral: true }).catch(e => { })
      ...
      .setDescription('** Исследуйте ритм, выберите свою звуковую магию ниже!**')
...
if(!filters?.includes(button?.customId)) return
      ...
      if (!filtre) return interaction?.editReply({ content: '❌ Неверное название', ephemeral: true }).catch(e => { })
     filtre = filtre?.toLowerCase()
      ...
      if (filters?.includes(filtre?.toLowerCase())) {
        ...
        embed?.setDescription(`❌ Такого фильтра нету!!`.replace("{filters}", filters?.map(mr => `\`${mr}\``).join(", ")))

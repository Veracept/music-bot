...
module.exports = {
  name: "play",
  description: "Приходи, послушаем музыку!!",
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!name) return interaction.reply({ content: `❌ Введите допустимое название песни.`, ephemeral: true }).catch(e => { });
      let res;
      try {
        ...
        return interaction.editReply({ content: `❌ Без результатов` }).catch(e => { });
      }

      if (!res || !res.length || !res.length > 1) return interaction.reply({ content: `❌ Без результатов`, ephemeral: true }).catch(e => { });
          ...
          switch (button.customId) {
            ...
            default: {
              ...
              try {
                ...
                await interaction.editReply({ content: `❌ Без результатов!`, ephemeral: true }).catch(e => { });
              }
              return collector.stop();
            }
          }
